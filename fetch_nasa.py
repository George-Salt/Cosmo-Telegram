...
def download_nasa_epic(token, epic_dir):
    epic_url = f"https://api.nasa.gov/EPIC/api/natural/images?api_key={token}"
    ...
    for image_num, image in enumerate(images_description[:10]):
        ...
        image_url = f"https://api.nasa.gov/EPIC/archive/natural/{epic_image_date}/png/{epic_image_name}.png?api_key={token}"
...
def download_nasa_apod(token, apod_dir):
    apod_url = f"https://api.nasa.gov/planetary/apod?api_key={token}"
    params = {
        "count": 30
    }
